...
class Pipeline:
    """Ergonomic risk analysis pipeline: stages 2 (features) → 3 (standards) → 5 (savings)."""
    ...
    def _stage2_features(self, session: SandingSession) -> ExposureFeatures:
        return extract_features(session, self.config, self.se_inputs.shift_duration_s)

    def _stage3_standards(self, features: ExposureFeatures) -> RiskScores:
        return evaluate(features, self.adapter)

    def _stage5_savings(self, scores: RiskScores) -> ESHResult:
